...
const templateSendOtpHTML = (code: string) => `
...
export const sendEmailOtp = async (
  ...
  const res = await resend.emails.send({
    from: "SUBETUARCHIVO <soporte@subetuarchivo.com>",
    to: [email],
    subject: "Código de verificación",
    html: templateSendOtpHTML(code),
  });
  return res;
};

export const sendEmailResetPass = async (
  email: string,
  url: string,
  name: string
): Promise<CreateEmailResponse> => {
  const res = await resend.emails.send({
    ...
    to: [email],
    subject: "Código de verificación",
    html: templateResetPasswordHTML(url, name),
  });
  return res;
};

export const templateResetPasswordHTML = (url: string, name: string) => `
  <head>
